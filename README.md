...
| `NEXT_PUBLIC_SITE_URL` | **Opcional.** URL pública p/ SEO (canonical, Open Graph, sitemap). Sem barra no final. Na Vercel a origem já vem do próprio deploy; defina só para sobrescrever (ex.: domínio próprio) — e só com a URL que realmente serve o site. |
...
2. Em **Settings → Environment Variables**, defina: `NEXT_PUBLIC_SUPABASE_URL`,
   ...
   `NEXT_PUBLIC_SITE_URL` é opcional — sem ela, o SEO usa a origem do próprio deploy;
...
| `NEXT_PUBLIC_SITE_URL` | **Optional.** Public URL for SEO (canonical, OG, sitemap). No trailing slash. On Vercel the origin is derived from the deploy itself; set this only to override it (e.g. a custom domain) — and only with the origin that actually serves the site. |
...
is optional — the SEO origin comes from the deploy unless you override it)
